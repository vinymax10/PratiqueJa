...
public class FilaGeracaoPostService implements Serializable
{
	...
	public void recuperarPendentes()
	{
		...
		for(PedidoPost pedido : pendentes)
		{
			...
			{
				desistidos++;
				logger.warning("Pedido de posts " + pedido.getId() + " abandonado: "
				...
				try
				{
					pedidoPostDAO.marcarErro(pedido.getId());
				}
				catch(Exception e)
				{
		...
		if(desistidos > 0)
			logger.warning(desistidos + " pedido(s) de posts abandonado(s) por excesso de tentativas.");

		if(recuperados > 0)
		{
			logger.info(recuperados + " pedido(s) de posts recuperado(s) na fila após início da aplicação.");
			disparar();
		}
	}
