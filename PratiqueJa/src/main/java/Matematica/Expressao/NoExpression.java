...
public class NoExpression
{
	...
	public NoExpression(String texto)
	{
		if(texto.equals("="))
		{
			literal = texto;
	...
	{
		return "NoExpression2:\tcoeficiente: " + coeficiente + "\t"
	...
	public String texto(boolean comSinal)
	{
		...
		if(tipoNo == TipoNo.Literal)
		{
			...
			{
				if(Math.abs(coeficiente.numerador)==1&&coeficiente.denominador != 1)
					string +=  "\\dfrac{"+literal+"}{"+ coeficiente.denominador+"}";
				else
	...
	public boolean igual(NoExpression x)
	{
		if(tipoNo == x.tipoNo)
			return true;
		else
