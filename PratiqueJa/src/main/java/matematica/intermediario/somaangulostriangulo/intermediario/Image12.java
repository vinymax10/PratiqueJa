...
public class Image12 extends Conta
{
	...
	public Image12(int indice)
	{
		...
		resultadoCorreto = "" + c + "°";
		resolucaoLatex = ResolucaoSomaAngulosTriangulo1.resolucaoSuplemento(a, b, c);
...
//		Graphics.addLabel(g2, strA, 173, 27, Alinhamento.TopCenter);
		
		formula = new TeXFormula(strB);
		icon = formula.new TeXIconBuilder().setStyle(TeXConstants.STYLE_DISPLAY).setSize(18).build();
		icon.setInsets(new Insets(0, 0, 0, 0));
		icon.paintIcon(null, g2, 30, 112 - icon.getIconHeight());

		formula = new TeXFormula("x");
		icon = formula.new TeXIconBuilder().setStyle(TeXConstants.STYLE_DISPLAY).setSize(18).build();
		icon.setInsets(new Insets(0, 0, 0, 0));
		icon.paintIcon(null, g2, 147, 103 - icon.getIconHeight());
