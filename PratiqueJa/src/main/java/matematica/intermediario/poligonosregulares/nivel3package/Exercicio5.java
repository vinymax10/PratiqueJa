...
public class Exercicio5 extends GeradorExercicio
{
	...
	protected void construir()
	{
		...
		addResolucao("Passo 1: calcular \\(P\\) e o apótema \\(a\\) do quadrado");
		addResolucao("\\(" + ResolucaoPoligonoRegular.formulaPerimetro() + "\\)");
		addResolucao("\\(P = 4 \\times " + l + " = " + p + ", \\quad a = \\dfrac{" + l + "}{2} = " + aApotema + "\\)");
		addResolucao("Passo 2: calcular \\(A\\)");
		addResolucao("\\(" + ResolucaoPoligonoRegular.formulaArea() + "\\)");
		addResolucao("\\(A = \\dfrac{" + p + " \\times " + aApotema + "}{2} = \\mathbf{" + area + "}\\)");
	}
}
