	...
	protected void construir()
	{
		...
		switch (tipo)
		{
			case 0:
				...
				break;
			case 1:
				addParagrafo(Nomes.feminino(rand) + " tinha \\(" + a + "\\) reais e gastou \\(" + b + "\\) reais numa livraria. Com quanto ela ficou?");
				break;
			case 2:
				addParagrafo("Uma fazenda colheu \\(" + a + "\\) kg de laranja. Foram entregues \\(" + b + "\\) kg ao mercado. Quantos kg restaram?");
				break;
			case 3:
				addParagrafo("Numa corrida, " + Nomes.masculino(rand) + " percorreu \\(" + a + "\\) km. Após \\(" + b + "\\) km, quantos km ainda faltavam para chegar?");
				break;
			default:
