...
public class Exercicio8 extends GeradorExercicio
{
	...
	{
		// 2.º quadrante: tg<0 → dado tg=-p/q, calcular cos via sec²=tg²+1; cos<0
		// Triplas: {p,q,r} → tg=-p/q, cos=-q/r
		...
		addParagrafo("Dado que \\(\\operatorname{tg}\\,\\theta = -\\dfrac{" + p + "}{" + q
				+ "}\\) e \\(\\theta\\) está no 2.º quadrante, calcule \\(\\cos\\theta\\).");
