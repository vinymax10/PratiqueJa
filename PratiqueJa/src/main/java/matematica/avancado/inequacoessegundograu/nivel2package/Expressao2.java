...
// Quadratic inequality with a < 0 (concavidade para baixo), two positive integer roots
public class Expressao2 extends GeradorExercicio
{
	...
	protected void construir()
	{
		...
		addResolucao("\\(x=\\dfrac{" + negB + "\\pm " + sqrtDelta + "}{" + twoA + "}\\)");
		// For a<0: x1=r1 uses +sqrt, x2=r2 uses -sqrt
		addResolucao("\\(x_1=\\dfrac{" + negB + "+" + sqrtDelta + "}{" + twoA + "}="
				+ "\\dfrac{" + (negB + sqrtDelta) + "}{" + twoA + "}=" + r1 + "\\)");
		addResolucao("\\(x_2=\\dfrac{" + negB + "-" + sqrtDelta + "}{" + twoA + "}="
				+ "\\dfrac{" + (negB - sqrtDelta) + "}{" + twoA + "}=" + r2 + "\\)");
		if (pedirPositivo)
		{
			addResolucao("Como \\(a=" + a + "<0\\), a parábola abre para baixo: \\(f(x)" + sinal + "0\\) entre as raízes.");
			addResolucao("\\(\\mathbf{x \\in " + abre + r1 + ",\\," + r2 + fecha + "}\\)");
		}
		else
		{
			addResolucao("Como \\(a=" + a + "<0\\), a parábola abre para baixo: \\(f(x)" + sinal + "0\\) fora das raízes.");
